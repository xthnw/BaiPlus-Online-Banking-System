...
if (isset($_POST['update'])) {
    ...
    if (isset($_FILES['bank_pic']) && $_FILES['bank_pic']['error'] == 0) {
        ...
        $sql = $conn->prepare("UPDATE `bank` SET bank_id = :id, 
                    bank_name = :bank_name, 
                    bank_pic = :bank_pic
                    WHERE bank_id = :id");
        ...
        overflow: hidden;
                ...
                <div class="page-transition-slide-in-right">
                    <div class="modal-body">
                        ...
                        <form action="editbank.php" method="POST" enctype="multipart/form-data">
                            <!-- <div class="modal-body">    ลดช่องบรรทัด enter -->

                            <?php
                            if (isset($_GET['id'])) {
                                $id = $_GET['id'];
                                $stmt = $conn->query("SELECT * FROM bank WHERE bank_id = '$id'");
                                $stmt->execute();
                                $data = $stmt->fetch();
                            }
                            ?>
                            <div class="form-group">
                                ...
                                <input type="text" value="<?php echo $data['bank_id']; ?>" class="form-control" name="id" required>
                                <input type="hidden" value="<?php echo $data['bank_pic']; ?>" class="form-control" name="bank_pic2">
                            </div>
                            <div class="form-group">
                                <label for="id"> Bank Name </label>
                                <input type="text" value="<?php echo $data['bank_name']; ?>" class="form-control" name="bank_name">
                            </div>

                            <div class="form-group" width="25px">
                                <label> Bank Logo ( .JPG .JPEG .PNG)</label>
                                <input type="file" name="bank_pic" class="form-control" id="imginput" onchange="previewImage(event)">
                                <img width="10%" id="imgpreview" src="bank-img/<?= $data['bank_pic'] ?>" class="">
                            </div>

                    </div>
                    <div class="modal-footer">
                        <a href="bank.php" type="button" class="btn btn-outline-secondary" data-dismiss="modal">Close</a>
