...
if ($param3 != 'Administrator') {
    echo "<script>alert('You don\'t have access or permission to this page!'); window.location.href = 'biller.php';</script>";
    exit();
}
...
if (isset($_POST['update'])) {
    ...
    if (isset($_FILES['biller_img']) && $_FILES['biller_img']['error'] == 0) {
        ...
        $sql = $conn->prepare("UPDATE `billerinfo` SET biller_id = :id, 
                    biller_name = :biller_name, 
                    biller_address = :biller_address,
                    postcode = :postcode,
                    biller_img = :biller_img
                    WHERE biller_id = :id");
        ...
        overflow: hidden;
                ...
                <div class="page-transition-slide-in-right">
                    <div class="modal-body">
                        ...
                        <form action="editbiller.php" method="POST" enctype="multipart/form-data">
                            <!-- <div class="modal-body">    ลดช่องบรรทัด enter -->

                            <?php
                            if (isset($_GET['id'])) {
                                $id = $_GET['id'];
                                $stmt = $conn->query("SELECT * FROM billerinfo WHERE biller_id = '$id'");
                                $stmt->execute();
                                $data = $stmt->fetch();
                            }
                            ?>
                            ...
                            <div class="form-group">
                                <label for="id"> Biller ID </label>
                                <input type="text" readonly value="<?php echo $data['biller_id']; ?>" class="form-control" name="id" required>
                                <input type="hidden" value="<?php echo $data['biller_img']; ?>" class="form-control" name="biller_img2">
                            </div>

                            <div class="form-group">
                                <label for="id"> Biller Name </label>
                                <input type="text" value="<?php echo $data['biller_name']; ?>" class="form-control" name="biller_name">
                            </div>
                            <div class="form-row">
                                <div class="form-group col">
                                    <label for="id">Biller Address</label>
                                    <input type="text" value="<?php echo $data['biller_address']; ?>" class="form-control" name="biller_address">
                                </div>
                                <div class="form-group col">
                                    ...
                                    <select name="postcode" class="form-control">
                                        ...
                                        while ($row = mysqli_fetch_assoc($result)) {
                                            ...
                                            $selected = ($customer_postcode == $data['postcode']) ? "selected" : "";
                            ...
                            <div class="form-group" width="25px">
                                <label> Biller Image ( .JPG .JPEG .PNG)</label>
                                <input type="file" name="biller_img" class="form-control" id="imginput" onchange="previewImage(event)">
                                <img width="10%" id="imgpreview" src="biller-img/<?= $data['biller_img'] ?>" class="">
                            </div>

                    </div>
                    <div class="modal-footer">
                        <a href="biller.php" type="button" class="btn btn-outline-secondary" data-dismiss="modal">Close</a>
