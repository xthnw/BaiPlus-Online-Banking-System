...
if ($param3 != 'Administrator') {
    echo "<script>alert('You don\'t have access or permission to this page!'); window.location.href = 'bill.php';</script>";
    exit();
}
...
if (isset($_POST['update'])) {
    ...
    $sql = $conn->prepare("UPDATE `bill` SET bill_id = :id, 
                    bill_exp = :bill_exp,
                    bill_amount = :bill_amount,
                    bill_owner_fname = :bill_owner_fname,
                    bill_owner_lname = :bill_owner_lname,
                    biller_id = :biller_id,
        ...
        overflow: hidden;
                ...
                <div class="page-transition-slide-in-right">
                    <div class="modal-body">
                        ...
                        <form action="editbill.php" method="POST" enctype="multipart/form-data">
                            <!-- <div class="modal-body">    ลดช่องบรรทัด enter -->

                            <?php
                            if (isset($_GET['id'])) {
                                $id = $_GET['id'];
                                $stmt = $conn->query("SELECT * FROM bill WHERE bill_id = '$id'");
                                $stmt->execute();
                                $data = $stmt->fetch();
                            }
                            ?>
                            <div class="form-group">
                                ...
                                <input type="text" readonly value="<?php echo $data['bill_id']; ?>" class="form-control" name="id" required>
                            </div>

                            <div class="form-group">
                                <label>Bill Expire</label>
                                <input type="datetime-local" value="<?php echo $data['bill_exp']; ?>" class="form-control" name="bill_exp">
                            </div>
                            <div class="form-group">
                                <label>Bill Amount</label>
                                <input type="number" step="0.01" oninput="truncateDecimals(this, 2);" name="bill_amount" value="<?php echo $data['bill_amount']; ?>" class="form-control" step="0.01" placeholder="Enter Bill Amount" oninput="truncateDecimals(this, 2);">
                            </div>
                            <div class="form-group">
                                <label>Bill Owner First Name</label>
                                <input type="text" value="<?php echo $data['bill_owner_fname']; ?>" class="form-control" name="bill_owner_fname">
                            </div>
                            <div class="form-group">
                                <label>Bill Owner Last Name</label>
                                <input type="text" value="<?php echo $data['bill_owner_lname']; ?>" class="form-control" name="bill_owner_lname">
                            </div>
                            <div class="form-group">
                                <label>Biller ID</label>
                                <input type="text" value="<?php echo $data['biller_id']; ?>" class="form-control" name="biller_id">
                            </div>
                            <div class="form-group">
                                <label>Account ID</label>
                                <input type="text" value="<?php echo $data['account_id']; ?>" class="form-control" name="account_id">
                            </div>
                    </div>
                    <div class="modal-footer">
                        <a href="bill.php" type="button" class="btn btn-outline-secondary" data-dismiss="modal">Close</a>
