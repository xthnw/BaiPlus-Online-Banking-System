...
if ($param3 != 'Administrator') {
    echo "<script>alert('You don\'t have access or permission to this page!'); window.location.href = 'transaction.php';</script>";
    exit();
}
...
if (isset($_POST['update'])) {
    ...
    $account_transferor = $_POST['account_transferor'];
    $account_receiver = $_POST['account_receiver'];
    $err_code = $_POST['err_code'];
    $trans_type = $_POST['trans_type'];
    $bill_id = $_POST['bill_id'];
    $loan_id = $_POST['loan_id'];

    $sql = $conn->prepare("UPDATE `transaction` SET transaction_id = :id, 
                    transaction_date = :transaction_date, 
                    transaction_amount = :transaction_amount, 
                    transaction_detail = :transaction_detail, 
                    account_transferor = :account_transferor, 
    ...
    $sql->bindParam(":transaction_detail", $transaction_detail);
    $sql->bindParam(":account_transferor", $account_transferor);
    $sql->bindParam(":account_receiver", $account_receiver);
    $sql->bindParam(":err_code", $err_code);
    $sql->bindParam(":trans_type", $trans_type);
        ...
        overflow: hidden;
                ...
                <div class="page-transition-slide-in-right">
                    <div class="modal-body">
                        ...
                        <form action="edittransaction.php" method="POST" enctype="multipart/form-data">
                            <!-- <div class="modal-body">    ลดช่องบรรทัด enter -->

                            <?php
                            if (isset($_GET['id'])) {
                                $id = $_GET['id'];
                                $stmt = $conn->query("SELECT * FROM `transaction` WHERE transaction_id = '$id'");
                                $stmt->execute();
                                $data = $stmt->fetch();
                            }
                            ?>
                            <div class="form-group">
                                ...
                                <input type="text" readonly value="<?php echo $data['transaction_id']; ?>" class="form-control" name="id" required>
                            </div>

                            <div class="form-row">
                                <div class="form-group col">
                                    <label>Transaction Date</label>
                                    <input type="datetime-local" value="<?php echo $data['transaction_date']; ?>" class="form-control" name="transaction_date">
                                </div>
                                <div class="form-group col">
                                    <label>Transaction Amount</label>
                                    <input type="number" step="0.01" oninput="truncateDecimals(this, 2);" name="transaction_amount" class="form-control" value="<?php echo $data['transaction_amount']; ?>">
                                </div>
                            </div>
                            <div class="form-group">
                                <label>Transaction Detail</label>
                                <input type="text" name="transaction_detail" class="form-control" value="<?php echo $data['transaction_detail']; ?>">
                            </div>
                            <div class="form-group">
                                ...
                                <select name="account_transferor" class="form-control">
                                    ...
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        ...
                                        $selected = ($account_id == $data['account_transferor']) ? "selected" : "";
                                        echo "<option value='$account_id' $selected>$account_id - $account_name</option>";
                                    }
                                    mysqli_close($conn);
                                    ?>
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select name="account_receiver" class="form-control">
                                    ...
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        $account_id = $row['account_id'];
                                        $account_name = $row['account_name'];
                                        $selected = ($account_id == $data['account_receiver']) ? "selected" : "";
                                        echo "<option value='$account_id' $selected>$account_id - $account_name</option>";
                                    }
                                    mysqli_close($conn);
                                    ?>
                            ...
                            <div class="form-group">
                                ...
                                <select name="err_code" class="form-control">
                                    ...
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        ...
                                        $err_desc = $row['err_desc'];
                                        $selected = ($err_code == $data['err_code']) ? "selected" : "";
                                        echo "<option value='$err_code' $selected>$err_code - $err_desc</option>";
                                    }
                                    mysqli_close($conn);
                                    ?>
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="trans_type" class="form-control" value="<?php echo $data['trans_type']; ?>">
                            </div>
                            <div class="form-group">
                                <label>Bill ID</label>
                                <select name="bill_id" class="form-control">
                                    ...
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        $bill_id = $row['bill_id'];
                                        $bill_owner_fname = $row['bill_owner_fname'];
                                        $selected = ($bill_id == $data['bill_id']) ? "selected" : "";
                                        echo "<option value='$bill_id' $selected>$bill_id - $bill_owner_fname</option>";
                                    }
                                    mysqli_close($conn);
                                    ?>
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <select name="loan_id" class="form-control">
                                    ...
                                    while ($row = mysqli_fetch_assoc($result)) {
                                        $loan_id = $row['loan_id'];
                                        $selected = ($loan_id == $data['loan_id']) ? "selected" : "";
                                        echo "<option value='$loan_id' $selected>$loan_id </option>";
                                    }
                                    mysqli_close($conn);
                                    ?>
                                </select>
                    ...
                    <div class="modal-footer">
                        <a href="transaction.php" type="button" class="btn btn-outline-secondary" data-dismiss="modal">Close</a>
