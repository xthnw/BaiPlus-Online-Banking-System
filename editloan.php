...
if (isset($_POST['update'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM loan WHERE loan_id = :id");
    ...
    $change = 0;
    if ($_POST['loan_amount'] != $row['loan_amount']) {
        $changes_loan_amount = $row['loan_amount'] . '->' . $_POST['loan_amount'] . ',';
        $change = 1;
    }
    if ($_POST['loan_duration'] != $row['loan_duration']) {
        $changes_loan_duration = $row['loan_duration'] . '->' . $_POST['loan_duration'] . ',';
        $change = 1;
    }
    if ($_POST['loan_interest'] != $row['loan_interest']) {
        $changes_loan_interest = $row['loan_interest'] . '->' . $_POST['loan_interest'] . ',';
        $change = 1;
    }

    if ($change == 1) {
        $sum_change =  $changes_loan_amount . $changes_loan_duration . $changes_loan_interest;

        $sql = $conn->prepare("UPDATE `loan` SET loan_amount = :loan_amount, 
        ...
        $sql = $conn->prepare("INSERT INTO `managehistory` (`datetime`, employee_id, account_id, action_type, loan_id)  VALUE (NOW(), :employee_id, :account_id, :sum_change, :loan_id)");
        $sql->bindParam(":employee_id", $param6);
        $sql->bindParam(":account_id", $account_id);
        $sql->bindParam(":sum_change", $sum_change);
        $sql->bindParam(":loan_id", $id);
        $sql->execute();
    }
    if ($change == 1) {
        ...
        overflow: hidden;
                ...
                <div class="page-transition-slide-in-right">
                    <div class="modal-body">
                        ...
                        <form action="editloan.php" method="POST" enctype="multipart/form-data">
                            <!-- <div class="modal-body">    ลดช่องบรรทัด enter -->

                            <?php
                            if (isset($_GET['id'])) {
                                $id = $_GET['id'];
                                $stmt = $conn->query("SELECT * FROM loan WHERE loan_id = '$id'");
                                $stmt->execute();
                                $data = $stmt->fetch();
                            }
                            ?>
                            <div class="form-group">
                                ...
                                <input type="text" readonly value="<?php echo $data['loan_id']; ?>" class="form-control" name="id" required>
                            </div>
                            <div class="form-row">
                                <div class="form-group col">
                                    <label> Account ID </label>
                                    <input type="text" name="account_id" value="<?php echo $data['account_id']; ?>" class="form-control" placeholder="Enter Account ID" readonly>
                                </div>
                                <div class="form-group col">
                                    <label> Loan Type </label>
                                    <select name="loan_type" class="form-control">
                                        <?php if ($data['loan_type'] === "Personal Loan") : ?>
                                            <option value="Personal Loan" selected>Personal Loan</option>
                                        <?php else : ?>
                                            <option value="Personal Loan">Personal Loan</option>
                                        <?php endif; ?>

                                        <?php if ($data['loan_type'] === "Business Loan") : ?>
                                            <option value="Business Loan" selected disabled>Business Loan</option>
                                        <?php else : ?>
                                            <option value="Business Loan" disabled>Business Loan</option>
                                        <?php endif; ?>

                                        <?php if ($data['loan_type'] === "Home Loan") : ?>
                                            <option value="Home Loan" selected disabled>Home Loan</option>
                                        <?php else : ?>
                                            <option value="Home Loan" disabled>Home Loan </option>
                                        <?php endif; ?>

                                        <?php if ($data['loan_type'] === "Car Loan") : ?>
                                            <option value="Car Loan" selected disabled>Car Loan</option>
                                        <?php else : ?>
                                            <option value="Car Loan" disabled>Car Loan</option>
                                        <?php endif; ?>
                                    </select>
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="form-group col">
                                    ...
                                    <input type="number" step="0.01" oninput="truncateDecimals(this, 2);" name="loan_amount" value="<?php echo $data['loan_amount']; ?>" class="form-control" placeholder="Enter Loan Amount">
                                </div>
                                <div class="form-group col">
                                    <label> Loan Start Date </label>
                                    <input type="date" name="loan_start_date" value="<?php echo $data['loan_start_date']; ?>" class="form-control" placeholder="Enter Loan Start Date" readonly>
                                </div>
                                <div class="form-group col">
                                    <label> Loan Duration </label>
                                    <select name="loan_duration" class="form-control">
                                        <?php if ($data['loan_duration'] === '12') : ?>
                                            <option value="12" selected>12</option>
                                        <?php else : ?>
                                            <option value="12">12</option>
                                        <?php endif; ?>

                                        <?php if ($data['loan_duration'] === '18') : ?>
                                            <option value="18" selected>18</option>
                                        <?php else : ?>
                                            <option value="18">18</option>
                                        <?php endif; ?>

                                        <?php if ($data['loan_duration'] === '24') : ?>
                                            <option value="24" selected>24</option>
                                        <?php else : ?>
                                            <option value="24">24 </option>
                                        <?php endif; ?>

                                        <?php if ($data['loan_duration'] === '36') : ?>
                                            <option value="36" selected>36</option>
                                        <?php else : ?>
                                            <option value="36">36</option>
                                        <?php endif; ?>

                                        <?php if ($data['loan_duration'] === '48') : ?>
                                            <option value="48" selected>48</option>
                                        <?php else : ?>
                                            <option value="48">48</option>
                                        <?php endif; ?>

                                        <?php if ($data['loan_duration'] === '60') : ?>
                                ...
                                <div class="form-group col">
                                    ...
                                    <input type="number" step="0.01" oninput="truncateDecimals(this, 2);" name="loan_interest" value="<?php echo $data['loan_interest']; ?>" class="form-control" placeholder="Enter Loan Interest">
                                </div>
                            </div>
                    </div>
                    <div class="modal-footer">
                        <a href="loan.php" type="button" class="btn btn-outline-secondary" data-dismiss="modal">Close</a>
