...
if (isset($_POST['update'])) {
    ...
    $sql = $conn->prepare("UPDATE `postcode` SET customer_postcode = :id, 
                customer_city = :customer_city, 
                customer_district = :customer_district
                WHERE customer_postcode = :id");
    $sql->bindParam(":id", $id);
    $sql->bindParam(":customer_city", $customer_city);
    $sql->bindParam(":customer_district", $customer_district);

    // Add the code to echo the query and bound variables here
    echo $sql->queryString;
    print_r([$id, $customer_city, $customer_district]);
...
<body id="page-top">
    ...
    .collapse {
        ...
        overflow: hidden;
                ...
                <div class="page-transition-slide-in-right">
                    <div class="modal-body">
                        ...
                        <form action="editpostcode.php" method="POST" enctype="multipart/form-data">
                            <!-- <div class="modal-body">    ลดช่องบรรทัด enter -->

                            <?php
                            if (isset($_GET['id'])) {
                                $id = $_GET['id'];
                                $stmt = $conn->query("SELECT * FROM postcode WHERE customer_postcode = '$id'");
                                $stmt->execute();
                                $data = $stmt->fetch();
                            }
                            ?>
                            <div class="form-group">
                                ...
                                <input type="text" readonly value="<?php echo $data['customer_postcode']; ?>" class="form-control" name="id" required>
                            </div>
                            <div class="form-row">
                                <div class="form-group col">
                                    <label> City </label>
                                    <input type="text" name="customer_city" value="<?php echo $data['customer_city']; ?>" class="form-control" placeholder="Enter Customer City">
                                </div>
                                <div class="form-group col">
                                    <label> District </label>
                                    <input type="text" name="customer_district" value="<?php echo $data['customer_district']; ?>" class="form-control" placeholder="Enter Customer District">
                                </div>

                            </div>
                    </div>
                    <div class="modal-footer">
                        <a href="postcode.php" type="button" class="btn btn-outline-secondary" data-dismiss="modal">Close</a>
        ...
        <script>
            ...
            const loanInterestInput = document.querySelector('input[name="loan_interest"]');

            let lastValidLoanAmount = '';
            let lastValidLoanInterest = '';
            ...
            loanAmountInput.addEventListener('input', function() {
                ...
                if (!regex.test(currentValue)) {
                    ...
                    this.value = lastValidLoanAmount; // set the input field value to the last valid value
                } else {
                    lastValidLoanAmount = currentValue; // update the last valid value
                }
            });

            loanInterestInput.addEventListener('input', function() {
                const currentValue = this.value;
                const regex = /^\d+(\.\d{0,2})?$/; // regex pattern to allow up to 2 decimal places

                if (!regex.test(currentValue)) {
                    alert("Please enter a valid number with up to 2 decimal places."); // show an error message
                    this.value = lastValidLoanInterest; // set the input field value to the last valid value
                } else {
                    lastValidLoanInterest = currentValue; // update the last valid value
                }
            });
        </script>
</body>
