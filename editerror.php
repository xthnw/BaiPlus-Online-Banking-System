...
if ($param3 != 'Administrator') {
    echo "<script>alert('You don\'t have access or permission to this page!'); window.location.href = 'error.php';</script>";
    exit();
}
...
if (isset($_POST['update'])) {
    ...
    $sql = $conn->prepare("UPDATE `error` SET err_code = :id, 
                    err_desc = :err_desc
                    WHERE err_code = :id");
        ...
        overflow: hidden;
                ...
                <div class="page-transition-slide-in-right">
                    <div class="modal-body">
                        ...
                        <form action="editerror.php" method="POST" enctype="multipart/form-data">
                            <!-- <div class="modal-body">    ลดช่องบรรทัด enter -->

                            <?php
                            if (isset($_GET['id'])) {
                                $id = $_GET['id'];
                                $stmt = $conn->query("SELECT * FROM error WHERE err_code = '$id'");
                                $stmt->execute();
                                $data = $stmt->fetch();
                            }
                            ?>
                            <div class="form-group">
                                <label for="id"> Error Code </label>
                                <input type="text" readonly value="<?php echo $data['err_code']; ?>" class="form-control" name="id" required>
                            </div>

                            <div class="form-group">
                                <label>Error Description</label>
                                <input type="text" value="<?php echo $data['err_desc']; ?>" class="form-control" name="err_desc">
                            </div>
                    </div>
                    <div class="modal-footer">
                        <a href="error.php" type="button" class="btn btn-outline-secondary" data-dismiss="modal">Close</a>
